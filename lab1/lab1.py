...
child_rule = IF ( AND('parent (?x) (?y)', NOT('child (?y) (?x)')), THEN('child (?y) (?x)'))
sibling_rule = IF ( AND('parent (?x) (?y)', 'parent (?x) (?z)', NOT(OR('duplicate (?y) (?z)', 'duplicate (?z) (?y)'))), THEN('sibling (?y) (?z)', 'sibling (?z) (?y)'))
...
family_rules = [duplicate_rule, child_rule, sibling_rule, grandparent_rule, cousin_rule]
    ...
    for rule in rules:
        ...
        if bound_conseq is not None:
            ...
            for ant in rule.antecedent():
                new_antes.append(backchain_to_goal_tree(rules, populate(ant, bound_conseq)))
