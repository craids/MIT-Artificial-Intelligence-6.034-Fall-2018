...
def sort_by_heuristic(graph, goalNode, nodes):
    ...
    class Heunode:
        def __init__(self, node, heu):
            self.node = node
            self.heu = heu
    heuristics = graph.heuristic_dict[goalNode]
    sorted_heunodes = list()
    for n in nodes:
        sorted_heunodes.append(Heunode(n, heuristics[n]))
    return [n.node for n in sorted(sorted_heunodes, key=lambda n: (n.heu, n.node))]
    ...
    while stack:
        ...
        if vertex not in visited:
            if vertex is goalNode:
                return make_generic_search(extensions, has_loops)()(graph, startNode, goalNode)
            visited.add(vertex)
...
def basic_bfs(graph, startNode, goalNode):
    ...
    exists, otherwise returning None.
    """
    return generic_search(do_nothing_fn, False, do_nothing_fn, False)(graph, startNode, goalNode)
...
def sort_new_paths_by_heuristic(graph, goalNode, paths):
    return sorted(paths, key=lambda path: (graph.get_heuristic_value(path[-1], goalNode), path))

def sort_new_paths_by_heuristic_and_edge_length(graph, goalNode, paths):
    return sorted(paths, key=lambda path: (graph.get_heuristic_value(path[-1], goalNode) + path_length(graph, path), path))

def sort_agenda_by_path_length(graph, goalNode, paths):
    queue = list()
    for p in paths:
        length = path_length(graph, p)
        p.append(length)
        queue.append(p)
    sorted_q = sorted(queue, key=lambda p: p[-1])
    for p in sorted_q:
        del p[-1]
    return sorted_q

def sort_agenda_by_path_length_and_heuristic(graph, goalNode, paths):
    queue = list()
    for p in paths:
        length = path_length(graph, p)
        p.append(length)
        queue.append(p)
    sorted_q = sorted(queue, key=lambda p: p[-1] + graph.get_heuristic_value(p[-2], goalNode))
    for p in sorted_q:
        del p[-1]
    return sorted_q
...
generic_hill_climbing = [sort_new_paths_by_heuristic, True, do_nothing_fn, False]

generic_best_first = [do_nothing_fn, False, sort_new_paths_by_heuristic, False]

generic_branch_and_bound = [do_nothing_fn, False, sort_agenda_by_path_length, False]

generic_branch_and_bound_with_heuristic = [sort_new_paths_by_heuristic_and_edge_length, False, sort_agenda_by_path_length_and_heuristic, False]

generic_branch_and_bound_with_extended_set = [do_nothing_fn, False, sort_agenda_by_path_length, True]

generic_a_star = [sort_new_paths_by_heuristic_and_edge_length, False, sort_agenda_by_path_length_and_heuristic, True]
...
def sort_agenda_with_beam(graph, goalNode, paths, beam_width):
    queue = list(paths)
    sorted_q = sorted(queue, key=lambda p: graph.get_heuristic_value(p[-1], goalNode))
    return sorted_q[0:beam_width]

generic_beam = [do_nothing_fn, False, sort_agenda_with_beam, False]
...
def is_admissible(graph, goalNode):
    ...
    for n in graph.nodes:
        if graph.get_heuristic_value(n, goalNode) > path_length(graph, generic_search(*generic_branch_and_bound_with_extended_set)(graph, n, goalNode)):
            return False
    return True
...
def is_consistent(graph, goalNode):
    ...
    for n in graph.nodes:
        for x in graph.get_neighbors(n):
            edgelen = graph.get_edge(n,x).length
            hdiff = abs(graph.get_heuristic_value(n, goalNode) - graph.get_heuristic_value(x, goalNode))
            if hdiff > edgelen:
                return False
    return True
...
WHAT_I_FOUND_INTERESTING = 'overarching concepts'
WHAT_I_FOUND_BORING = 'generic search'
SUGGESTIONS = 'beam impl unclear'
